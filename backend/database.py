...
# - connect_args sets busy_timeout (milliseconds) so concurrent writes
#   queue up instead of immediately raising "database is locked"
...
async def _apply_wal_mode(connection):
    ...
    WAL allows simultaneous reads while a write is in progress, which eliminates
    most 'database is locked' errors when add-item and stock-adjust fire together.
    """
